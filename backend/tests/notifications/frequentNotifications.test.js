...
describe("Frequent Notifications", () => {
    ...
    it("should not create a notification if one already exists within threshold", async () => {
        ...
        const result = await shouldCreateNotification(defaultUser.userId, "Test message");
        expect(result).toBe(false);
    });

    it("should create a notification if none exists within threshold", async () => {
        const result = await shouldCreateNotification(defaultUser.userId, "Another test message");
    ...
    it("should respect a custom threshold", async () => {
        ...
        const result = await shouldCreateNotification(defaultUser.userId, "Custom threshold message", 100);
        expect(result).toBe(false);
    });
    ...
    it("should generate overdue notification for task that is past due", async () => {
        ...
        const overdueNotifs = await generateOverdueNotifications(defaultUser.userId, now);
