...
const { generateWeeklyInsights } = require("../../scheduler/weeklyNotifications");
...
describe("Weekly Notifications", () => {
    ...
    it("should generate notifications and update lastWeeklyNotification when tasks completed last week meet conditions", async () => {
        ...
        const notifications = await Notification.find({ userId: defaultUser.userId });
        ...
        expect(new Date(updatedUser.lastWeeklyNotification) >= startOfThisWeek).toBe(true);
    });
    ...
    it("should not generate notifications if one was already sent this week", async () => {
        ...
        startOfThisWeek.setDate(startOfThisWeek.getDate() - diffToMonday);
        startOfThisWeek.setHours(0, 0, 0, 0);
        await User.findOneAndUpdate({ userId: defaultUser.userId }, { lastWeeklyNotification: startOfThisWeek });
    ...
    it("should generate a task count notification if 1 task was completed last week", async () => {
        ...
        const taskCountNotif = notifications.find(n => n.message === expectedMessage);
    ...
    it("should generate insight with new high-priority tasks message", async () => {
        ...
        await generateWeeklyInsights();
        const notifications = await Notification.find({ userId: defaultUser.userId });
        const expectedMessage = "Weekly Insight: Your high priority tasks accounted for only 20% of your total time last week, with some new tasks added recently. Consider reviewing them for improved focus.";
        const insightNotif = notifications.find(n => n.message === expectedMessage);
    ...
    it("should generate insight with no new high-priority tasks message", async () => {
        ...
        await generateWeeklyInsights();
        const notifications = await Notification.find({ userId: defaultUser.userId });
        const expectedMessage = "Weekly Insight: You spent only 20% of your time on high priority tasks last week. Consider increasing your focus on these tasks.";
        const insightNotif = notifications.find(n => n.message === expectedMessage);
        expect(insightNotif).toBeDefined();
    });
    ...
    it("should generate insight with 49% time spent on high priority tasks", async () => {
        ...
        await generateWeeklyInsights();
        const notifications = await Notification.find({ userId: defaultUser.userId });
        const expectedMessage = "Weekly Insight: You spent only 49% of your time on high priority tasks last week. Consider increasing your focus on these tasks.";
        const insightNotif = notifications.find(n => n.message === expectedMessage);
        expect(insightNotif).toBeDefined();
    });
    ...
    it("should generate a great job insight when highPercentage is exactly 50%", async () => {
        ...
        await generateWeeklyInsights();
        const notifications = await Notification.find({ userId: defaultUser.userId });
        const expectedMessage = "Weekly Insight: Great job! You dedicated 50% of your time to high priority tasks last week.";
        const insightNotif = notifications.find(n => n.message === expectedMessage);
        expect(insightNotif).toBeDefined();
    });
    ...
    it("should generate a great job insight when highPercentage is 100%", async () => {
        ...
        await generateWeeklyInsights();
        const notifications = await Notification.find({ userId: defaultUser.userId });
        const expectedMessage = "Weekly Insight: Great job! You dedicated 100% of your time to high priority tasks last week.";
        const insightNotif = notifications.find(n => n.message === expectedMessage);
        expect(insightNotif).toBeDefined();
    });
    ...
    it("should not create duplicate notifications if one exists within the threshold", async () => {
        ...
        const duplicateMessage = "Weekly Insight: Your high priority tasks accounted for only 20% of your total time last week, with some new tasks added recently. Consider reviewing them for improved focus.";
        ...
        await generateWeeklyInsights();
        const notifications = await Notification.find({ userId: defaultUser.userId });
        const insightNotifs = notifications.filter(n => n.message === duplicateMessage);
